...
export const editInstructorInfoThunk = instructor => async dispatch => {
  try {
    console.log(instructor)
    let res = await axios.put(`${path}/instructors/${instructor.instructorId}`, instructor);
...
export const editCourseInfoThunk = course => async dispatch => {
  try {
    console.log(course)
    let res = await axios.put(`${path}/courses/${course.instructorId}`, course);
    //res.data is the updated course object
    console.log(res.data, "data")
    dispatch(ac.editCourse(course));
  } catch(err) {
    console.error(err);
  }
};
